...
class TitanBot extends Client {
  ...
  startWebServer() {
    ...
    app.use((req, res, next) => {
      ...
      times.push(now); requestCounts.set(ip, times); next();
    });
    ...
    const startServer = (port, attempt = 0) => {
      ...
      const server = app.listen(port, host, () => { hasStartedListening = true; this.webServer = server; startupLog(`✅ Web Server running on ${host}:${port}`); });
      server.on('error', (error) => {
        if (!hasStartedListening && error?.code === 'EADDRINUSE' && attempt < maxPortRetryAttempts) { const nextPort = port + 1; startupLog(`Port ${port} is already in use. Trying port ${nextPort}...`); setTimeout(() => startServer(nextPort, attempt + 1), 250); return; }
        logger.error(`❌ Web server error on port ${port}: ${error?.message || error}`); if (!hasStartedListening) process.exit(1);
      });
    };
  ...
  async loadHandlers() {
    ...
    for (const handler of handlers) {
      try {
        ...
        await loaderFn(this); startupLog(`✅ Loaded ${handler.path}`);
      } catch (error) {
