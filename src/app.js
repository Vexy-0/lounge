...
function loadData() {
  ...
  for (const [guildId, value] of Object.entries(raw)) {
    ...
    cfg.gifOnly = {
      ...
      users: Array.isArray(value?.gifOnly?.users) ? value.gifOnly.users : [],
    };
...
function isGifUrl(value) {
  ...
  if (DISCORD_MEDIA.some(h => host === h || host.endsWith(`.${h}`))) {
    return /(?:format=gif|fm=gif|ext=gif|type=gif|\.gif(?:$|[?#]))/i.test(url);
  }
  return false;
}
...
const LOG_TYPES = {
  ...
  moderation: ['🛡️ Moderation Logs', 'moderation-logs'],
...
async function runAutoMod(message) {
  ...
  if (cfg.spam) {
    ...
    const now = Date.now();
    const history = (spam.get(key) || []).filter(t => now - t < cfg.spamWindow);
    history.push(now);
    spam.set(key, history.slice(-10));
    if (history.length >= cfg.spamCount) return punish(message, 'Message spam');
  }
}
...
function resolveTarget(guild, raw) {
  const id = getTargetId(raw);
  const role = guild.roles.cache.get(id);
  if (role) return { type: 'role', id: role.id, name: role.name };
  const member = guild.members.cache.get(id);
  if (member) return { type: 'user', id: member.id, name: member.user.tag };
  return null;
}
...
async function handleSlash(i) {
  ...
  if (i.commandName === 'sticky') {
    ...
    const sent = await i.channel.send({ content, allowedMentions: { parse: [] } });
  ...
  if (i.commandName === 'stickyremove') {
    ...
    await i.channel.messages.delete(old.messageId).catch(() => {});
  ...
  if (i.commandName === 'automod') {
    ...
    if (target.type === 'role') {
      if (action === 'add' && !cfg.gifOnly.roles.includes(target.id)) cfg.gifOnly.roles.push(target.id);
      if (action === 'remove') cfg.gifOnly.roles = cfg.gifOnly.roles.filter(id => id !== target.id);
    } else {
